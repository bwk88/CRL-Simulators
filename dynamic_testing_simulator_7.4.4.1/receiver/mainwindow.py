...
# from PyQt5.QtCore import QMenu

tree = ET.parse('config/configuration.xml')
root = tree.getroot()
file_path = './blocked_msgs/file.txt'
file_path_blk = './config/msg_list.txt'
msg_CSCI_path = './config/msg_CSCI.txt'
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.pushButtonExit.setStyleSheet("background:lightblue")
        self.pushButtonExit.setObjectName("pushButtonExit")
        self.pushButtonExit.setToolTip("Click to see the list of all Messages that are added")
        ...
        self.pushButtonFilter.setToolTip("Clears the UI view")
        ...
        self.pushButtonLiveOffline.setToolTip("Enable This Button to show only the messages that are added in the List")
        self.pushButtonLiveOffline.setStyleSheet("background:lightblue;")
        self.horizontalLayout_4.addWidget(self.pushButtonLiveOffline)
        
        
        self.addMsg = QtWidgets.QPushButton(self.tabAllSamples)
        self.addMsg.setObjectName("addMsg")
        self.addMsg.setToolTip("Add Messages that you want to allow to be shown in UI")
        self.addMsg.setStyleSheet("background:lightblue;")
        
        self.msgListLineEdit = QtWidgets.QLabel(self.tabAllSamples)
        self.msgListLineEdit.setObjectName("msgListLineEdit")
        
        
        
        self.horizontalLayout_4.addWidget(self.pushButtonExit)
        self.horizontalLayout_4.addWidget(self.addMsg)
        self.horizontalLayout_4.addWidget(self.msgListLineEdit)
        ...
        self.blockedComboBoxMessages.setFixedWidth(300)
        ...
        self.pushButtonLoad.setToolTip("Block after selecting message from comboBox")
        self.pushButtonLoad.setStyleSheet("background:red;")
        ...
        self.unBlockButton.setToolTip("Unblock Message after selecting from combobox")
        # self.unBlockButton.setStyleSheet("background:red;")
        self.horizontalLayout_2.addWidget(self.unBlockButton)
        
        self.filterSrc = QtWidgets.QComboBox(self.tabSingleSample)
        self.filterSrc.setObjectName("filterSrc")
        self.filterSrc.setEditable(True)
        self.horizontalLayout_2.addWidget(self.filterSrc)
        
        self.filterDst = QtWidgets.QComboBox(self.tabSingleSample)
        self.filterDst.setObjectName("filterDst")
        self.filterDst.setEditable(True)
        self.horizontalLayout_2.addWidget(self.filterDst)
        
        self.filterBtn = QtWidgets.QPushButton(self.tabAllSamples)
        self.filterBtn.setCheckable(True)
        self.filterBtn.setText("Filter")
        self.filterBtn.setObjectName("unBlockButton")
        self.filterBtn.setToolTip("Filter the message based on SRC and DST")
        # self.unBlockButton.setStyleSheet("background:red;")
        ...
        self.addMsg.clicked.connect(self.addMsgtoList)
        ...
        # self.tableView.customContextMenuRequested.connect(self.show_msg_list)
        ...
        self.pushButtonExit.clicked.connect(self.show_msg_list)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButtonExit.setText(_translate("MainWindow", "Allowed Message List "))
        self.addMsg.setText(_translate("MainWindow", "Add Messages"))
        ...
        self.pushButtonLiveOffline.setText(_translate("MainWindow", "Only Show Selected Msg"))
    ...
    def show_msg_list(self,  pos: QtCore.QPoint):
        try:
            print("Show")
            # index = self.msgTable.indexAt(pos)
            dialog = QtWidgets.QDialog(self.tableView)
            dialog.setWindowTitle("Enabled Messages")
            ...
            self.msgTable = QtWidgets.QTableView()
            layout.addWidget(self.msgTable)
        
            
            self.new_model = QtGui.QStandardItemModel()
            self.new_model.setHorizontalHeaderLabels(['Enableds Messages'])
            
            
            # print("Index",index)
            
            for val in self.msg_enable:
                items = [QtGui.QStandardItem(f"{val}")]
                self.new_model.appendRow(items)
                
            self.msgTable.setModel(self.new_model)
            self.msgTable.setColumnWidth(0, 650)
            self.msgTable.setColumnWidth(1, 950)
            
            self.msgTable.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
            self.msgTable.customContextMenuRequested.connect(self.deletMsg)
            
            
            # index = self.msgTable.selectionModel().selectedIndexes()
            # self.dltBtn.clicked.connect(lambda:self.deletMsg(index))
            dialog.show()
    ...
    def deletMsg(self,pos: QtCore.QPoint):
        index = self.msgTable.indexAt(pos)
        print("indexx",index,"pos",pos)
        if index.isValid():
            menu = QtWidgets.QMenu(self.msgTable)
            delete_action = QtWidgets.QAction('Delete Message',self.msgTable)
            menu.addAction(delete_action)
            action = menu.exec_(self.msgTable.viewport().mapToGlobal(pos))
            
            if action == delete_action:
                txt = self.new_model.data(index)
                self.new_model.removeRow(index.row())
                # -----------------Can be optimized---------------
                try:
                    for i,item in enumerate(self.msg_enable):
                        if self.msg_enable[i] == txt:
                            self.msg_enable.pop(i)
                            
                            
                            
                except Exception as e:
                    print("Error while deleting from Dialog",e)
                    print("Error while deleting from list",e)
                    
                    
                with open(file_path_blk,'w',encoding="utf-8") as file:
                    for val in self.msg_enable:
                        file.write(val+'\n')      
                        
                self.addMsg.setText("Add Messages("+str(len(self.msg_enable))+")s")      
        ...
        try:
            ...
            for index in selected_indexes:
                ...
                for column in range(datastore.model.columnCount()):
                    ...
                    if item:
                        # print("SSSSSSSSSSSSSSSSSSSitem.text()",item.text());
                        row_data.append(item.text())
                print('Selected row_data:', row_data,"rd",row_data[6]) 
    
                dialog = QtWidgets.QDialog(self.tableView)
                #dialog.setWindowTitle(f"{row_data[0]} {row_data[1]}")
                dialog.setWindowTitle(f"{row_data[0]}")
                dialog.resize(650, 900)
                layout = QtWidgets.QVBoxLayout()
                dialog.setLayout(layout)
                ...
                new_model.setHorizontalHeaderLabels(['Field','Value'])
                
                if row_data[6] != 'UNKNOWN': #or row_data[4] == 'MSG LEN Failed':
                    data = row_data[7]
                    print("DATA>>>>>",row_data[7])
                    if(row_data[6]!=''):
                        ...
                        print("DATA INDEX complete_tuple",complete_tuple)
                        content_list = []
                        
                        if len(complete_tuple) == len(rcvd_format):    
                            value_list = byte_data_parsing(complete_tuple,rcvd_format)
                        # else:
                        # print("seleted_msg_label_list",row_data)
                        
                        for x in seleted_msg_label_list:
                            print("--",x)
                        try:
                            for i,value in enumerate(seleted_msg_label_list):
                                # print("------------->>>>>",value)
                                #items = [QtGui.QStandardItem(f"{seleted_msg_label_list[i]}"),QtGui.QStandardItem(f"{complete_tuple[i]}")]
                                items = [QtGui.QStandardItem(f"{seleted_msg_label_list[i]}"),QtGui.QStandardItem(f"{complete_tuple[i]}")]
    ...
    def add_entry(self, time, msg, hex_string,content, labels, observation_str,content_format,hex_data,data):
        ...
        #reading mapping of CSCI ID and Name from text file msg_CSCI.txt presemt in config folder 
        module_name = {}
       
        try:
            with open(msg_CSCI_path,'r',encoding="utf-8") as f:
                print("-----------------====================",f)
                for i,v in enumerate(f):
                    module_name[f"{i+1}"] = v[:-1]
            
            print("module_name",module_name,header_data[0],header_data[1])
    
            #finding the name on the basis of CSCI ID received in header
            SRC_INDEX_IN_HEADER = 0
            DST_INDEX_IN_HEADER =1
        
        except Exception as e:
            print("FILE READING ERROR",e)

        try:
            src_text = module_name[str(header_data[SRC_INDEX_IN_HEADER])]
        except Exception as e:
            src_text = 'NA'
        try:
            dst_text = module_name[str(header_data[DST_INDEX_IN_HEADER])]
        except Exception as e:
            dst_text = 'NA'
        ...
        if config.mode == 'live':
            ...
            items = [QtGui.QStandardItem(f"{time}"),QtGui.QStandardItem(f"{config.dynamic_header_format}"),QtGui.QStandardItem(f"{header_data[0]} ({src_text})"),QtGui.QStandardItem(f"{header_data[1]} ({dst_text})"),QtGui.QStandardItem(f"{msg}"),obs, QtGui.QStandardItem(f"{content}"),QtGui.QStandardItem(f"{hex_string}"),QtGui.QStandardItem(f"{labels}"),QtGui.QStandardItem(f"{content_format}")]
            
        if self.filterBtn.isChecked():
            rcvd_hdr = msg.split('_')

            if(msg not in self.blocked_msgs and rcvd_hdr[0] == self.filterSrc.currentText() and rcvd_hdr[1] == self.filterDst.currentText()):
        ...
        elif msg in self.blocked_msgs:
            print("message needs to be blocked")
        elif self.pushButtonLiveOffline.isChecked():
            if msg in self.msg_enable:
                print("enable",msg)
                datastore.model.insertRow(0, items)
                self.data_index.insert(0,hex_string)
                self.hex_index.insert(0,hex_data)
    ...
    def load_message(self):
        ...
        with open(msg_CSCI_path,'r',encoding="utf-8") as f:
            for i,v in enumerate(f):
                self.filterSrc.addItem(v.strip())
                self.filterDst.addItem(v.strip())
                # print(v.strip())
    ...
    def unblock_msg(self):
        try:
            ...
            # self.msgCountLineEdit.setText("Blocked msg(s): " + str(len(self.blocked_msgs)))
            self.pushButtonLoad.setText("Block Selected ("+str(len(self.blocked_msgs))+")")
        except Exception as e:
    ...
    def load_blocked_msgs(self):
        try:
            ...
            with open(file_path_blk,'r',encoding="utf-8") as file:
                for val in file:
                    self.msg_enable.append(val[:-1])
                # print("msssgh ena",self.msg_enable)
            self.addMsg.setText("Add Messages("+str(len(self.msg_enable))+")")
            ...
            self.pushButtonLoad.setText("Block Selected ("+str(len(self.blocked_msgs))+")")
            # config.blocked_msg_logs = self.blocked_msgs.copy()
        except Exception as e:
            print("load_blocked_msgs Error")
        
    def reset_blocked(self):
        try:
            print("hoihwef",self.blockedComboBoxMessages.currentText(),self.pushButtonLiveOffline.isChecked())
            
            if self.pushButtonLiveOffline.isChecked():
                print("Checked",self.pushButtonLiveOffline.isChecked())
            else:
                self.pushButtonLiveOffline.setCheckable(True)
                print("Checked",self.pushButtonLiveOffline.isChecked())
    ...
    def addMsgtoList(self):
        msg_text=self.blockedComboBoxMessages.currentText()  
        if msg_text not in self.msg_enable:    
            self.msg_enable.append(msg_text)
            with open(file_path_blk,'w',encoding="utf-8") as file:
                for val in self.msg_enable:
                    file.write(val+'\n')
        else:
            print(msg_text,"already present")
        self.addMsg.setText("Add Messages("+str(len(self.msg_enable))+")")
