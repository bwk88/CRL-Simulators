...
def csv_dict_func(csv_file):
    ...
    for index ,bitName in enumerate(df['MESSAGE_NAME']):
        # print(bitName)
        if bitName == 'ATSS_PC_ATC_VIRTUAL_BLOCK_COMMAND_PACKET':
            print('bitnaaaaammm')
            if pd.notnull(bitName): #checking wether the cell is empty or not
                ...
                msg_lst = [arg_name, arg_type, arr_size, arg_size, input_val, format_val, bit_field, irs_data, struct_name, struct_array_size]
    
                if pd.isnull(arg_name):
                    continue
                
                # dict['ATSS_PC_ATC_VIRTUAL_BLOCK_COMMAND_PACKET'].append(msg_lst)
        ...
        # for i,item in enumerate(dict['ATSS_PC_ATC_VIRTUAL_BLOCK_COMMAND_PACKET']):
        #     print(i,"-------------",item)
        # print_dict()
