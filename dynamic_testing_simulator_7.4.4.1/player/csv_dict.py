...
def csv_dict_func(csv_file):
    ...
    for index ,bitName in enumerate(df['MESSAGE_NAME']):
        ...
        if pd.notnull(bitName): #checking wether the cell is empty or not
            ...
            struct_array_size = df.at[index,'ATR_ARRAY_SIZE']
            ...
            if pd.isnull(static_array_length):
                ...
                static_array_length = int(df.at[index, 'STATIC_ARRAY_SIZE'])

            bit_field = int(bit_field) 
            msg_lst = [arg_name, arg_type, arr_size, arg_size, input_val, format_val, bit_field, irs_data, struct_name, struct_array_size, static_array_length]
            ...
            csv_dict[msg_name].append(msg_lst)

    for key,items in csv_dict.items():
        names.append(key)
    # print("DICTIONARY",csv_dict['ATSS_PC_ATC_VIRTUAL_BLOCK_COMMAND_PACKET'],len(csv_dict['ATSS_PC_ATC_VIRTUAL_BLOCK_COMMAND_PACKET']))
    print("CSV to DICT Successful")
    # for i,item in enumerate(csv_dict['ATSD_ATSS_HEADWAY_MODE_SETTING']):
    #     print(i,"-------------",item[0])
    print_dict()
